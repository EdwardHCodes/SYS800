...
cns3xxx_i2c_xfer_msg(struct i2c_adapter *adap, struct i2c_msg *msg)
{
    ...
    {
        printk("%s:%d: Presently the driver does not handle extended addressing\n",
    ...
    for (i = 0; i < msg->len; i++)
    {
        ...
        if (wait_event_timeout(i2c->wait, (i2c->state == STATE_ERROR) || (i2c->state == STATE_DONE), TWI_TIMEOUT))
        {
